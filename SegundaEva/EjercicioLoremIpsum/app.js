...
let numberInput = document.getElementById("amount");

form.addEventListener("submit", (ev) => {
  //Eliminar párrafos antiguos
  let oldParagraphs = document.getElementsByClassName("generatedParagraph");
  [...oldParagraphs].forEach(oldParagraph => {
    document.body.removeChild(oldParagraph);
  });

  //Generar Nuevos párrafos
  let defNumber = numberInput.value;
  for (let i = 0; i < defNumber; i++) {
    let nextLorem = text[Math.floor(Math.random() * 9)];
    let nextParagraph = document.createElement("p");
    nextParagraph.innerText = nextLorem;
    nextParagraph.setAttribute("class", "generatedParagraph");
